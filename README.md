...
## <a name="p2">2. Установка Redis як Native Service на Raspberry PI5</a>

Redis використовується як розділєма пам'ять між додатками, що забезпечує збереженя стану додатків навіть після їх перезавантаження. Повідмолення в телеграм теж відправляються класично, через чергу Redis  на окремий додаток. Для економії писанини на SD-карту Redis ставиться не в контейнері, а окремо, як сервіс. Також тут описані параметри доконфігурації додатка, щоб працювати з  чергами з віддаленої машини.
...
```text
save 60 1  # Зберігати на диск, якщо пройшло 60 сек і змінився хоча б 1 раз
```

Але, зважаючи що у нас SD-карта і повідомлення ідуть десь раз за 5 хвилин то можна поставити кешування 300 секунд 
...
У мене налаштовано підключення до Redis без авторизації. Це налаштування важливо для можливості підключення з віддаленої машини. Якщо цього не зробити то для підключення з віддаленої машини прийдеться встановити пароль
...
3. В автоматичних сервісах потрібно докрурити залежності так, щоб сервіс запускався уже після  Redis
